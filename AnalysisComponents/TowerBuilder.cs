    ...
    public class TowerBuilder : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            pManager.AddRectangleParameter(
                "Plot",
                "P",
                "The boundary rectangle of the plot.",
                GH_ParamAccess.item
            );
            ...
            pManager.AddIntegerParameter(
                "Division Steps",
                "DS",
                "Number of operations to apply false output will be a solid tower.",
                GH_ParamAccess.item
            );
            pManager.AddBooleanParameter(
                "Reduce",
                "R",
                "When true it allows the reduction of some voxels from the tower.",
                GH_ParamAccess.item
            );
            pManager.AddIntegerParameter(
                "Reduction Max",
                "RDMX",
                "The number of voxels to be deleted.",
                GH_ParamAccess.item,
                1
            );
            pManager.AddIntegerParameter(
                "Rotation Max",
                "RDMX",
                "The number of voxels to be rotated.",
                GH_ParamAccess.item,
                1
            );
        }
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            pManager.AddBoxParameter(
                "Tower",
                "T",
                "The final voxelized tower.",
                GH_ParamAccess.list
            );
            pManager.AddTextParameter(
                "Tower Info",
                "TI",
                "Basic information for the output tower.",
                GH_ParamAccess.item
            );
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetData(0, ref Plot))
                return;
            if (!DA.GetData(1, ref BCR))
                return;
            if (!DA.GetData(2, ref FAR))
                return;
            if (!DA.GetData(3, ref Division_Steps))
                return;
            if (!DA.GetData(4, ref Reduce))
                return;
            if (!DA.GetData(5, ref Reduction_max))
                return;
            if (!DA.GetData(6, ref Rotation_max))
                return;
            ...
            if (Final_Tower.Count > 0)
            {
                ...
                info =
                    floors.ToString()
                    + " floors tower, with total height of "
                    + (floors * 4.5).ToString()
                    + "m. and "
                    + Final_Tower.Count.ToString()
                    + " voxels.";
            }
            else
            {
        ...
        //this method return the initial boundary volumeeeeee
        ...
        public List<Box> Divide(Box Start)
        {
            ...
            bool intersection = Rhino.Geometry.Intersect.Intersection.BrepPlane(
                bstart,
                pl,
                0.001,
                out myArrayC,
        ...
        public List<Box> RandomReduce(List<Box> DividedTower, int reduction_max)
        {
            ...
            var indexes = Enumerable
                .Range(0, DividedTower.Count - 1)
                .OrderBy(x => rnd.Next())
                .Take(DividedTower.Count - reduction_max)
                .ToList();
